...
from app.schemas.meat_product import MeatProductCreate, MeatProductList
from app.models.category import Category
from sqlalchemy import text

def list_meat_products_db(db: Session, skip: int = 0, limit: int = 100 
) -> list[MeatProductList]:
    products = db.query(MeatProduct).filter(
...
def get_meat_product_by_id(db: Session, id: str) -> MeatProduct:
    product = db.query(MeatProduct).filter(
        MeatProduct.id == id, 
